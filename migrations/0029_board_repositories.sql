...
CREATE INDEX idx_board_repositories_repository ON board_repositories(repository_id);

INSERT OR IGNORE INTO board_repositories (board_id, repository_id, created_at)
SELECT board_id, repository_id, COALESCE(MIN(created_at), datetime('now'))
FROM tasks
WHERE repository_id IS NOT NULL
GROUP BY board_id, repository_id;

INSERT OR IGNORE INTO board_repositories (board_id, repository_id, created_at)
SELECT board_id, repository_id, COALESCE(created_at, datetime('now'))
FROM board_maintainers
WHERE repository_id IS NOT NULL;

DROP INDEX IF EXISTS idx_board_maintainers_repository;
...
INSERT INTO board_maintainers_next (
  ...
  name, prompt, interval_seconds, status, last_run_at, last_ama_session_id, last_error_message, created_at, updated_at
)
SELECT
  id, owner_id, board_id, agent_id, ama_schedule_id, ama_http_trigger_id, ama_memory_store_id,
  name, prompt, interval_seconds, status, last_run_at, last_ama_session_id, last_error_message, created_at, updated_at
