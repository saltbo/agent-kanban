/**
 * Dispatcher — full task dispatch pipeline + agent environment helpers.
 *
...
import { apiCallIdempotent, apiCallOptional, cryptoBoundary, fsSync } from "./boundaries.js";
...
export function buildAgentEnv(opts: BuildEnvOpts): Record<string, string> {
  const { agentId, sessionId, agentName, agentUsername } = opts;
...
export async function dispatchTasks(
  ...
  const dispatched = await dispatchOne(task, dir, boardType, client, pool, opts.machineId);
...
async function dispatchOne(
  ...
  const created = await apiCallIdempotent("createSession", () => client.createSession(agentId, sessionId, (pubKeyJwk as JsonWebKey).x!, machineId));
