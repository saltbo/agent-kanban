...
export function registerTaskEventRoutes(api: Hono<{ Bindings: Env }>): void {
  api.get("/api/tasks/:id/events", authorizeScope("task:read"), waitForEvents);
}
...
async function waitForEvents(c: TaskContext): Promise<Response> {
  ...
  if (!taskId || taskId.length > 200) {
    return v2Problem(c, 400, "invalid-task-event-query", "Invalid Task Event query", "taskId must identify one Task");
  }
