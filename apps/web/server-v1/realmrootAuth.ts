...
export async function endRealmrootWebSession(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  if (logoutUrl) {
    ...
    logoutUrl.searchParams.set("post_logout_redirect_uri", new URL("/", c.req.url).toString());
  }
...
export async function authenticateRealmrootToken(c: Context<{ Bindings: Env }>): Promise<RealmrootPrincipal> {
  ...
  const type =
    realmrootAgentId && actorProfile === "ai_agent" ? "agent" : topProfile === "service" ? "service" : topProfile === "machine" ? "machine" : "human";
