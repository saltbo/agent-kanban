...
type AgentStatus = "idle" | "working" | "done" | "rate_limited";
...
export function convertEvents(events: RuntimeEvent[], agentStatus: AgentStatus): ThreadMessageLike[] {
  ...
  function ensureCurrentTurn(re: RuntimeEvent) {
...
export function AmaRuntimeProvider({ events: rawEvents, taskDone, children }: AmaRuntimeProviderProps) {
  const events = useMemo(() => rawEvents.map(amaEventToRuntimeEvent), [rawEvents]);
...
export function amaEventToRuntimeEvent(raw: Record<string, unknown>): RuntimeEvent {
