...
describe("boardRepo", () => {
  ...
  it("listBoards returns boards for a specific owner", async () => {
    const { listBoards, createBoard } = await import("../server/adapters/d1/boardRepo");
  ...
  it("getBoardByName returns a board by name", async () => {
    const { getBoardByName } = await import("../server/adapters/d1/boardRepo");
  ...
  it("getBoard returns board with tasks array", async () => {
    const { createBoard, getBoard } = await import("../server/adapters/d1/boardRepo");
  ...
  it("records board repository mappings from dev tasks", async () => {
    const { createBoard } = await import("../server/adapters/d1/boardRepo");
    const { listBoardRepositories } = await import("../server/adapters/d1/boardRepositoryRepo");
  ...
  it("getDefaultBoard returns the first board for a user", async () => {
    const { getDefaultBoard } = await import("../server/adapters/d1/boardRepo");
  ...
  it("updateBoard updates name only", async () => {
    const { createBoard, updateBoard } = await import("../server/adapters/d1/boardRepo");
  ...
  it("updateBoard returns null for unknown board", async () => {
    const { updateBoard } = await import("../server/adapters/d1/boardRepo");
    const updated = await updateBoard(env.DB, "nonexistent", "board-test-user", { name: "X" });
    expect(updated).toBeNull();
  });

  it("deleteBoard removes a board", async () => {
    const { createBoard, deleteBoard, getBoard } = await import("../server/adapters/d1/boardRepo");
  ...
  it("deleteBoard returns false for unknown board", async () => {
    const { deleteBoard } = await import("../server/adapters/d1/boardRepo");
