...
describe("LandingPage v2 product copy", () => {
  it("presents Enbor, Enbor Runner, Realmroot Toolbox, and independent review without legacy architecture copy", () => {
    render(React.createElement(MemoryRouter, null, React.createElement(LandingPage)));

    expect(
      screen.getByText(
        /Enbor coordinates your Agents, and Enbor Runner hosts self-hosted execution\. Realmroot gives them identity and Toolbox access\./,
      ),
    ).toBeInTheDocument();
    ...
    expect(screen.getByText("Enbor Runtime Ownership")).toBeInTheDocument();
