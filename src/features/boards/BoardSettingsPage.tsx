...
import { BoardSettingsNav } from "@/features/boards/components/BoardSettingsNav";
import { Header } from "@/features/boards/components/Header";
import { useBoard, useDeleteBoard, useUpdateBoard } from "@/features/boards/hooks/useBoard";
...
function BoardSharingSection({ board, boardId }: { board: BoardSettingsBoard; boardId: string }) {
  ...
  async function copyShareText(type: "link" | "badge") {
    if (!shareUrl || !badgeBaseUrl) return;
    const text = type === "link" ? shareUrl : badgeMarkdown(shareUrl, badgeBaseUrl);
    try {
      await copyTextToClipboard(text);
      toast.success(type === "link" ? "Share link copied" : "Tasks badge copied");
    } catch {
...
function ShareLinks({ badgeBaseUrl, shareUrl, onCopy }: { badgeBaseUrl: string; shareUrl: string; onCopy: (type: "link" | "badge") => void }) {
  return (
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          <div className="min-w-0 flex-1">
            <img src={badgeBaseUrl} alt="AK tasks badge" className="h-5 max-w-full" />
          </div>
          <Button variant="outline" size="xs" onClick={() => onCopy("badge")}>
            Copy tasks
          </Button>
        </div>
      </div>
...
function badgeMarkdown(shareUrl: string, badgeUrl: string) {
  return `[![AK Tasks](${badgeUrl})](${shareUrl})`;
}
