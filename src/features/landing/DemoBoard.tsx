...
function assign(i: number) {
  return { assigned_to: A[i].id, assignee_name: A[i].name };
}
...
function makeTasks(): DemoTask[] {
  ...
  const t = (id: string, title: string, labels: string[], repo: string, ag?: number): DemoTask => ({
    ...
    ...(ag !== undefined ? assign(ag) : { assigned_to: null, assignee_name: null }),
    result: null,
...
function spawn(s: DS, ai: number, ti: number): DS {
  const av = new Map(s.avatars);
  av.set(A[ai].id, { agentId: A[ai].id, agentName: A[ai].name, identitySeed: A[ai].id, taskId: TID[ti], phase: "spawning" });
  return { ...s, avatars: av };
}
